...
export_graphviz(tree0, out_file="images/svg/19.heart_tree.dot", class_names=["악성", "양성"],
                feature_names=X_dafaframe.columns,
                impurity=False, filled=True)
with open("images/svg/19.heart_tree.dot", encoding='utf8') as f:
    dot_graph = f.read()

# display(graphviz.Source(dot_graph))
# print(graphviz.Source(dot_graph))
images.image.save_graph_as_svg(dot_graph, "19.heart_decision_tree")  
...
export_graphviz(tree4, out_file="images/svg/19.heart_tree_depth4.dot", class_names=["악성", "양성"],
                feature_names=X_dafaframe.columns,
                impurity=False, filled=True)
with open("images/svg/19.heart_tree_depth4.dot", encoding='utf8') as f:
    dot_graph = f.read()
...
images.image.save_graph_as_svg(dot_graph, "19.heart_decision_tree_depth4")  
...
images.image.plot_feature_importances(tree4, X_dafaframe, X_dafaframe.columns)
plt.title('heart 데이터로 학습시킨 결정 트리의 특성 중요도')
images.image.save_fig("19.heart_tree_depth4_feature_importances")  
plt.show()
...
images.image.save_graph_as_svg(tree_not_monotone, "19.heart_decision_tree_not_monotone")  

# 
plt.title('X[1]에 있는 정보만 사용 학습한 결정 트리')
images.image.save_fig("19.heart_decision_tree_not_monotone")  
plt.show()
