...
images.image.save_graph_as_svg(dot_graph, "3.cancer_decision_tree")  
...
images.image.save_graph_as_svg(dot_graph, "3.cancer_decision_tree_depth4")  
...
images.image.plot_feature_importances(tree4, cancer.data, cancer.feature_names)
plt.title('유방암 데이터로 학습시킨 결정 트리의 특성 중요도')
images.image.save_fig("3.cancer_tree_depth4_feature_importances")  
plt.show()
...
images.image.save_graph_as_svg(tree_not_monotone, "3.cancer_decision_tree_not_monotone")  

# 
plt.title('X[1]에 있는 정보만 사용 학습한 결정 트리')
images.image.save_fig("3.cancer_decision_tree_not_monotone")  
plt.show()
